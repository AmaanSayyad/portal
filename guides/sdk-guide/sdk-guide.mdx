...
// If you want to add other chains, search for their IDs and add them to the array below
...
If you have read functionalities that you want to integrate into your app or less frequent writing needs, the server-side is your choice.
...
An example where you write is your rules on claiming an NFT. Perhaps you got some conditions an user needs to comply with. This code is suited for your backend.
...
To use the SDK at the server-side, the object `sdk` is instantiated differently. It makes use of the **private keys** of a wallet. You can import the private keys, using the **dotenv** library. In the same folder, you would need a `.env` file to store the private keys.
