...
After you’re done this is what your Pack module should look like in the dashboard. I called my pack module `random`.
...
Go ahead and create a new Python file. Let's call it `python_nft.py` and open it in your favourite code editor. We'll use Visual Studio.
...
To start using the sdk, we will create an object inheriting all the methods inside the sdk.
...
In our `python_nft.py` we include the following code. First, we define a variable **pack_contract** and store our module address here.
...
Note, the Pack module only works with NFTs from the 'Drop' and 'Bundle' modules, because these modules are based on ERC-1155. If this is new to you, in short a Pack holds multiple NFTs. 
...
To open a Pack we can re-use most of the code. instead of the `create()` method, we will call the `open_pack` method 👇

You can find the ‘pack_id’ inside your dashboard of fetching the data by using the `get_all` method we used in step 7.
