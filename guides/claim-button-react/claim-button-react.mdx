...
In your `App.jsx` file we can use the code from this guide for our `connect wallet` button. thirdweb created a **component** for the `connect wallet` button so we don't have to code it ourselves. This is great so we don’t have to mess around with private keys later. 
