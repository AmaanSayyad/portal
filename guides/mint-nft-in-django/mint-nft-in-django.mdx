...
In our `nft_network.py` we include the following code. First, we define a variable `nft_smart_contract_address` and store our module address here.
...
If you go to the GitHub [repo](https://github.com/razacodespython/nft_creator), you might have noticed that I use an html form instead of a Django form. I did this because some projects might use Django solely for the backend and html forms at the front. However feel free to use Django forms, the process would be essentially the same!
