...
Currently, we provide you with components and hooks to easily integrate web3 into your app and set up an app-wide context without having to deal with the complexity of lower-level web3 configuration.
...
We will build our custom component that will allow you to authorise any action by **signing** it with your wallet.
...
First, let's create our homepage with the `Connect Wallet` component
...
Next up let's create our backend that will simulate our database on a server. For this, I'm going to use a docker image that builds a Redis database. Here is the yaml file if you want to do the same:
...
//we're doing everything locally, so set the host to 'local host'
...
Inside our folder `api` create a file `jws.ts`. Now we need to make sure our backend generates a JWT after the message has been signed and validated by our back-end. 
...
Finally, let's add the html tags and include the calls to the functions to get and update the username.
