...
Go ahead and create a new TypeScript file. Let's call it `index.ts` and open it in your favourite code editor. We'll use Visual Studio Code.
...
In our `index.ts` file we include the following code. First, we define a variable `marketModuleAddress` and store our module address here.
...
If you want to sell your NFT in your own coins, you will also need the module address of your coins. Head over to the dashboard and inside your project go to your `Token` module and copy the address of that module and paste it below.
...
We are using the NFT that we created in our previous guide. Each NFT has `tokenId`. We will pass the following parameters to the '.list' method:
